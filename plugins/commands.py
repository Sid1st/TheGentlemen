...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('✪ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/+fNGvkfXhsQVkZjI1"),
            InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/Movies_request_group_2024')
        ],[
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('Movie Request Gʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}'),
                        InlineKeyboardButton('Backup Cʜᴀɴɴᴇʟ', url=CHNL_LNK)
                    ],[
                        #InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/MoviesXonee")
                    ],[
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    InlineKeyboardButton('Movie Request Gʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}'),
                    InlineKeyboardButton('Backup Cʜᴀɴɴᴇʟʟ', url=CHNL_LNK)
                ],[
                    #InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/MoviesXonee")
                ],[
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MoviesXonee  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@MoviesXonee  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@MoviesXonee  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
...
async def shortlink(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(f"<b>Hey {message.from_user.mention}, This command only works on groups !\n\n<u>Follow These Steps to Connect Shortener:</u>\n\n1. Add Me in Your Group with Full Admin Rights\n\n2. After Adding in Grp, Set your Shortener\n\nSend this command in your group\n\n—> /shortlink ""{your_shortener_website_name} {your_shortener_api}\n\n#Sample:-\n/shortlink publicearn.com CAACAgUAAxkBAAEJ4GtkyPgEzpIUC_DSmirN6eFWp4KInAACsQoAAoHSSFYub2D15dGHfy8E\n\nThat's it!!! Enjoy Earning Money 💲\n\n[[[ Trusted Earning Site - https://publicearn.com]]]\n\nIf you have any Doubts, Feel Free to Ask me - @BadasSubham\n\n(Puriyala na intha contact la message pannunga - @BadasSubham)</b>")
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete :(\n\nGive me a shortener website link and api along with the command !\n\nFormat: <code>/shortlink publicearn.com e3d82cdf8f9f4783c42170b515d1c271fb1c4500</code></b>")
